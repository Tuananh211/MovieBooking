...
import com.base.moviebooking.databinding.SchedulleChildFragment1Binding;
...
public class ScheduleChildFragment2 extends BaseFragment<SchedulleChildFragment1Binding> {
    ScheduleChildModel2 mViewModel;
    ScheduleCinemaModel scheduleCinemaModel;
    ScheduleAdapter scheduleAdapter;

    @Override
    protected int getLayoutId() {
        return R.layout.schedulle_child_fragment1;
    }
    ...
    public void initView() {
        ...
        mViewModel = ViewModelProviders.of(this, viewModelFactory).get(ScheduleChildModel2.class);
        binding.rcvLichphim.setLayoutManager(new LinearLayoutManager(getContext(), RecyclerView.VERTICAL,false));
        String day = scheduleCinemaModel.day1.getValue().toString();
        ...
        mViewModel.dataMovie.observe(getViewLifecycleOwner(), new Observer<List<MovieSchedule>>() {
            ...
            public void onChanged(List<MovieSchedule> listMovieResponse) {
                if (listMovieResponse.size()!=0){
                    ...
                    View dialogLoadSchedule = getActivity().findViewById(R.id.dialog_load_schedule1);
                    if (dialogLoadSchedule != null) {
                        dialogLoadSchedule.setVisibility(View.GONE);
                    }
//                    getActivity().findViewById(R.id.dialog_load_schedule1).setVisibility(View.GONE);
